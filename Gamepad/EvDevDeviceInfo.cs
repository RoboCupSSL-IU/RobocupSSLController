...
using static RobocupSSLController.Gamepad.NativeUnsafeMethods;
using static RobocupSSLController.Gamepad.Ioctl;
namespace RobocupSSLController.Gamepad
{
    public unsafe class EvDevDeviceInfo
    {
        ...
        public Dictionary<EvDevAbsoluteAxis, EvDevAbsAxisInfo> AbsoluteAxes { get; set; }
        ...
        EvDevDeviceInfo(int fd, string path)
        {
            ...
            {
                AbsoluteAxes = abs.ToDictionary(x => (EvDevAbsoluteAxis) x, x =>
                {
        ...
        public EvDevGuessedDeviceType GuessDeviceType()
        {
            if (Name != null)
            {
                ...
                var isAbsolutePointingDevice = AbsoluteAxes?.ContainsKey(EvDevAbsoluteAxis.ABS_X) == true;
            ...
            if (Keys?.Contains(EvDevKey.BTN_LEFT) == true && Keys?.Contains(EvDevKey.BTN_RIGHT) == true)
            {
                ...
                if (AbsoluteAxes != null)
                {
                    if (AbsoluteAxes?.ContainsKey(EvDevAbsoluteAxis.ABS_X) == true)
                    {
        ...
        internal EventStream OpenEventStream()
        {
            return new EventStream(File.OpenRead(DevicePath));
        }

        public Gamepad OpenGamepad()
        {
